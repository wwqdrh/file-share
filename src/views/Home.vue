  ...
  <div class="body">
    ...
    <el-card class="file-list">
      ...
      <div style="padding-left: 16px; padding-right: 16px">
        <el-table :max-height="listHeight" :show-header="false" :data="files" row-key="id">
          <el-table-column width="30px" align="center">
            <template slot-scope="scope">
              <el-checkbox v-if="['directory', 'file'].includes(scope.row.type)"
                @change="onSelectHandler(scope.row.name)" :value="scope.row.selected"
            ...
            <template slot-scope="scope">
              <div :class="`${scope.row.type === 'directory' ? 'pointer' : ''} file-desc`"
                ...
                <el-tooltip class="filename" effect="light"
                  :content="scope.row.type === 'text' ? scope.row.content : scope.row.name" placement="right">
                  <div>{{ scope.row.name }}</div>
                </el-tooltip>
                <el-tooltip v-show="!!scope.row.username" effect="light" :content="`由【${scope.row.username}】分享`"
                  placement="top">
          ...
          <el-table-column width="70">
            <template slot-scope="scope">
              <el-button v-if="['file', 'directory'].includes(scope.row.type)"
                @click="handleDownload(scope.row, $event)" icon="el-icon-download" size="mini" plain></el-button>
              <el-button v-if="scope.row.type === 'text'" @click="handleDownload(scope.row, $event)"
                icon="el-icon-document-copy" size="mini" plain></el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
    </el-card>
    <el-dialog title="分享文件" name="file" customClass="dialog" :visible.sync="fileFormVisible">
      <div style="display: flex; justify-content: center">
        <el-upload drag action="/api/addFile" :on-success="uploadSuccess" :on-error="uploadError" :file-list="fileList"
    ...
    <el-dialog title="分享文本" customClass="dialog" :visible.sync="msgFormVisible">
    ...
    <el-dialog title="身份校验" customClass="dialog" :visible.sync="loginFormVisible">
...
export default {
  ...
  mounted() {
    ...
    // 更新请求头内容
    this.updateHeaders()
    fetchEventSource("/api/registrySSE", {
      onmessage() {
